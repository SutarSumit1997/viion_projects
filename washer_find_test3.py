...
outputText="READY"
var = StringVar()
l = Label(root, textvariable=var,wraplength=398,font=("times new roman",52,"bold"),bg="WHITE")
l.place(x=680,y=350)
var.set("")
...
def capture_image():
    ...
    gray_img=cv2.cvtColor(img_rgb_roi,cv2.COLOR_RGB2GRAY)
    #blurred = cv2.GaussianBlur(gray_img, (3, 3), 0)
    (T, threshInv) = cv2.threshold(gray_img, 10, 255,
	cv2.THRESH_BINARY_INV)
   # hsv = cv2.cvtColor(img_rgb_roi, cv2.COLOR_BGR2HSV)
    # define range of blue color in HSV 
    #lower_blue = np.array([13,30,30])
    #upper_blue = np.array([18,300,300])
